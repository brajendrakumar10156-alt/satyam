...
fn vs_main(@builtin(vertex_index) vIdx: u32, @builtin(instance_index) iIdx: u32) -> VertexOutput {
  ...
  let uvW = charData.uvInfo.z;
  let uvH = charData.uvInfo.w;
  let posW = charData.posInfo.z;
  let posH = charData.posInfo.w;
  
  let px = charData.posInfo.x + (q.x * posW);
  let py = charData.posInfo.y + (q.y * posH);
  
  let clipX = (px / uniforms.resolution.x) * 2.0 - 1.0;
  let clipY = 1.0 - (py / uniforms.resolution.y) * 2.0;
  out.position = vec4<f32>(clipX, clipY, 0.0, 1.0);
  
  let atlasSize = 512.0; // Handled by uv coordinates ratio mapping if dynamic, but we can rely on standard 512-1024
  let dynamicAtlasSize = uvW * 0.0 + 512.0; // If you wanted to pass it, but for now we'll just read from texture if possible, wait, we must use 1024 if size > 32
  // Actually, wait, let's fix atlasSize to 1024.0 in shader
  out.uv = vec2<f32>(
    (charData.uvInfo.x + q.x * uvW) / 1024.0,
    (charData.uvInfo.y + q.y * uvH) / 1024.0
  );
  return out;
}
...
fn fs_main(in: VertexOutput) -> @location(0) vec4<f32> {
  ...
  // Smoothly antialias the edge of the glyph using standard SDF thresholding
  let width = 0.05; // SDF transition width
  let alpha = smoothstep(0.5 - width, 0.5 + width, distance);
  if (alpha < 0.01) { discard; }
  return vec4<f32>(uniforms.textColor.rgb, uniforms.textColor.a * alpha); 
}
`;
...
@group(0) @binding(2) var imgTexture: texture_2d<f32>;

struct QuadData {
  pos: vec2<f32>,
  uv: vec2<f32>,
};
@group(0) @binding(3) var<storage, read> quads: array<QuadData>;
...
fn vs_main(@builtin(vertex_index) vIdx: u32) -> VertexOutput {
  var out: VertexOutput;
  let q = quads[vIdx];
  let clipX = (q.pos.x / uniforms.resolution.x) * 2.0 - 1.0;
  let clipY = 1.0 - (q.pos.y / uniforms.resolution.y) * 2.0;
  out.position = vec4<f32>(clipX, clipY, 0.0, 1.0);
  out.uv = q.uv;
  return out;
}
...
const WebGPUChartEngine = React.forwardRef((props, ref) => {
  const {
    candles = [],
  ...
  const [gpuDeviceName, setGpuDeviceName] = useState('Detecting GPU...');
  ...
  useEffect(() => {
    ...
    async function initWebGPU() {
      ...
      try {
        ...
        if (!adapter) throw new Error('WebGPU Adapter initialization failed. No adapter found.');

        // Extract Hardware & Discrete GPU Info (NVIDIA / AMD / Intel Arc auto-detection bridge)
        let activeGpuString = 'NVIDIA / HIGH-PERFORMANCE DISCRETE GPU';
        try {
          // 1. Try WebGL unmasked driver renderer first (detects NVIDIA GeForce on dual-GPU Acer Aspire 7)
          let detectedNvidia = false;
          try {
            const tempCanvas = document.createElement('canvas');
            const gl = tempCanvas.getContext('webgl2', { powerPreference: 'high-performance' }) 
              || tempCanvas.getContext('webgl', { powerPreference: 'high-performance' });
            if (gl) {
              const ext = gl.getExtension('WEBGL_debug_renderer_info');
              if (ext) {
                const unmaskedRenderer = gl.getParameter(ext.UNMASKED_RENDERER_WEBGL);
                if (unmaskedRenderer && (unmaskedRenderer.toLowerCase().includes('nvidia') || unmaskedRenderer.toLowerCase().includes('geforce') || unmaskedRenderer.toLowerCase().includes('rtx') || unmaskedRenderer.toLowerCase().includes('gtx'))) {
                  const cleanedName = unmaskedRenderer.replace(/^ANGLE \(([^,]+),\s*/, '').replace(/\s*Direct3D.*$/, '');
                  activeGpuString = `NVIDIA (${cleanedName})`;
                  detectedNvidia = true;
                }
              }
            }
          ...
          // 2. If WebGPU adapter info has specific vendor string
          if (!detectedNvidia) {
            let gpuVendor = 'Discrete GPU';
            let gpuArch = 'Hardware Accelerated';
            if ('info' in adapter && (adapter as any).info) {
              const info = (adapter as any).info;
              gpuVendor = info.vendor || info.architecture || 'Discrete GPU';
              gpuArch = info.architecture || info.device || info.description || 'Native WGSL Pipeline';
            } else if ('requestAdapterInfo' in adapter) {
              const info = await (adapter as any).requestAdapterInfo();
              gpuVendor = info.vendor || info.architecture || 'Discrete GPU';
              gpuArch = info.architecture || info.device || info.description || 'Native WGSL Pipeline';
            }
            activeGpuString = `${gpuVendor.toUpperCase()} (${gpuArch})`;
          }
        } catch (e) {
          console.warn('[WebGPU] Hardware info query warning:', e);
        }

        setGpuDeviceName(activeGpuString);
        console.log(`🚀 [WebGPU Engine] Active GPU Selected: ${activeGpuString}`);
        ...
        gpu.current.minUniformAlignment = device.limits?.minUniformBufferOffsetAlignment || 256;

        device.lost.then((info) => {
          console.warn('WebGPU device was lost:', info.message);
          isInitializedRef.current = false;
          gpu.current.deviceLost = true;
          setGpuError("WebGPU device lost. Re-initializing hardware engine...");
          setTimeout(() => {
            if (isMounted) initWebGPU();
          }, 1000);
        });
        ...
        const linePipeline = device.createRenderPipeline({
          ...
          fragment: {
            module: lineShaderModule, entryPoint: 'fs_main', targets: [{
              format, blend: {
        ...
        // Generate SDF at high-res 64px for sharp scalable text
        ...
        const fontTexture = device.createTexture({
          ...
          format: 'r8unorm', // Grayscale single channel is perfect for SDF
        ...
        device.queue.writeTexture(
          { texture: fontTexture },
          sdfData,
          { bytesPerRow: atlasSize, rowsPerImage: atlasSize },
        ...
        // ── LOGO WATERMARK PIPELINE ──
        try {
          const response = await fetch('/src/assets/logo.png');
          const blob = await response.blob();
          // Scale it down using browser's high quality scaler so WebGPU doesn't alias
          const imgBitmap = await createImageBitmap(blob, { resizeWidth: 512, resizeHeight: 512, resizeQuality: 'high' });
          ...
          gpu.current.logoTexture = logoTexture;
          
          const imageUniformBuffer = device.createBuffer({
            size: 32, // resolution(2), pad(2), tint(4)
            usage: GPUBufferUsage.UNIFORM | GPUBufferUsage.COPY_DST
          });
          gpu.current.imageUniformBuffer = imageUniformBuffer;
          
          const imageStorageBuffer = device.createBuffer({
            size: 6 * 16, // 6 vertices * 4 floats (pos2, uv2)
            usage: GPUBufferUsage.STORAGE | GPUBufferUsage.COPY_DST
          });
          gpu.current.imageStorageBuffer = imageStorageBuffer;

          const imageShaderModule = device.createShaderModule({ code: wgslImageShader });
          gpu.current.imagePipeline = device.createRenderPipeline({
            layout: 'auto',
            vertex: { module: imageShaderModule, entryPoint: 'vs_main' },
            fragment: {
              module: imageShaderModule, entryPoint: 'fs_main',
              targets: [{
                format, blend: {
                  color: { srcFactor: 'src-alpha', dstFactor: 'one-minus-src-alpha', operation: 'add' },
                  alpha: { srcFactor: 'one', dstFactor: 'one-minus-src-alpha', operation: 'add' }
          ...
          gpu.current.imageBindGroup = device.createBindGroup({
            layout: gpu.current.imagePipeline.getBindGroupLayout(0),
            entries: [
              { binding: 0, resource: { buffer: imageUniformBuffer } },
              { binding: 1, resource: imageSampler },
              { binding: 2, resource: logoTexture.createView() },
              { binding: 3, resource: { buffer: imageStorageBuffer } }
            ]
          });
        } catch(e) {
          console.error("Failed to load logo watermark", e);
        }
  ...
  // ── RENDER PIPELINE ────────────────────────────────────────────────────────
  const render = () => {
    if (!gpu.current.device || !gpu.current.pipeline || gpu.current.deviceLost) return;

    if (!gpuCanvasRef.current) return;
    const cw = gpuCanvasRef.current.width || (vState.current.width * dpr);
    const ch = gpuCanvasRef.current.height || (vState.current.height * dpr);
    if (cw <= 0 || ch <= 0) return;
    ...
    // ── Dynamic price step calculation first to get maxPriceW ──
    ...
    const getTextWidth = (str) => {
      ...
      if (!gpu.current.charMap) return 0;
    ...
    vState.current.pAxisW = pAxisW / dpr; // Save in CSS pixels for mouse events
    ...
    if (gpu.current.charMap && gpu.current.textStorageBuffer) {
      ...
      const pushText = (str, x, y) => {
        ...
        for (let i = 0; i < str.length; i++) {
          ...
          textData[off] = x - (paddingValue * fontScale); 
          textData[off + 1] = y - (map.h * fontScale) / 2; 
          textData[off + 2] = map.w * fontScale; // posW
          textData[off + 3] = map.h * fontScale; // posH
          
          textData[off + 4] = map.x; // uvX
          textData[off + 5] = map.y; // uvY
          textData[off + 6] = map.w; // uvW
          textData[off + 7] = map.h; // uvH
          x += charW;
          charCount++;
        }
      };
      ...
      const timeAxisY_initial = ch - (28 * dpr);

      // ── Dynamic price step calculation already done above ──
      const rawPriceLabels_temp = [];
      ...
      // ── Dynamic time height measurement ──
      ...
      const timeAxisH = maxTimeH + 16 * dpr; // 8px padding top and bottom
      const timeAxisY = ch - timeAxisH;
      vState.current.timeAxisH = timeAxisH / dpr; // Save in CSS pixels

      // ── Calculate vertical price winners using final timeAxisY ──
      const scaleY = timeAxisY / priceRange;
      ...
      priceAxisWinners.forEach(({ y, label }) => {
        ...
        pushText(label, Math.floor(labelX), Math.floor(y));
      });

      // ── Live Price Text (GPU Native) ──
      let livePixelY = -1.0;
      if (candles.length > 0) {
        const lastC = candles[candles.length - 1];
        const minP = vState.current.priceRange.min;
        const maxP = vState.current.priceRange.max;
        const priceScale = (maxP - minP) > 0 ? timeAxisY / (maxP - minP) : 1;
        livePixelY = timeAxisY - ((lastC.close - minP) * priceScale);
      ...
      // ── Hover Tooltip Texts (GPU Native) ──
      ...
      if (hoverPixel) {
        ...
        if (hx >= 0 && hx <= cw - pAxisW && hy >= 0 && hy <= timeAxisY) {
          // Calculate price at hover position
          const hoverPrice = max - (hy / scaleY);
          const hoverPriceStr = hoverPrice.toFixed(decPlaces);
          const wPrice = getTextWidth(hoverPriceStr);
          const labelX = cw - 8 * dpr - wPrice;
          pushText(hoverPriceStr, Math.floor(labelX), Math.floor(hy));
      ...
      if (charCount > 0) {
        ...
        const txtCol = darkMode ? [0.788, 0.820, 0.851, 1.0] : [0.075, 0.090, 0.133, 1.0];
        gpu.current.device.queue.writeBuffer(gpu.current.textUniformBuffer, 0, new Float32Array([cw, ch, 0, 0, ...txtCol]));
      }
    }
    ...
    // ── 0.5 DRAW WATERMARK LOGO ──
    if (gpu.current.imagePipeline && gpu.current.logoTexture) {
      const tint = darkMode ? [1.0, 1.0, 1.0, 0.05] : [0.0, 0.0, 0.0, 0.05];
      gpu.current.device.queue.writeBuffer(gpu.current.imageUniformBuffer, 0, new Float32Array([cw, ch, 0, 0, ...tint]));
      ...
      gpu.current.device.queue.writeBuffer(gpu.current.imageStorageBuffer, 0, logoData);
      
      renderPass.setPipeline(gpu.current.imagePipeline);
      renderPass.setBindGroup(0, gpu.current.imageBindGroup);
      renderPass.draw(6);
    }
    ...
    if (gpu.current.gridPipeline) {
      ...
      const cur_pAxisW = (vState.current.pAxisW || 65) * dpr;
      const cur_timeAxisH = (vState.current.timeAxisH || 28) * dpr;

      const gridSpacingX = Math.max(24 * dpr, (cw - cur_pAxisW) / 10);
      const gridSpacingY = Math.max(18 * dpr, (ch - cur_timeAxisH) / 8);
      ...
      if (candles && candles.length > 0) {
        ...
        const minP = vState.current.priceRange.min;
        const maxP = vState.current.priceRange.max;
        const priceScale = (maxP - minP) > 0 ? (ch - cur_timeAxisH) / (maxP - minP) : 1;
        livePixelY = (ch - cur_timeAxisH) - ((lastC.close - minP) * priceScale);

        const logicalRange = vState.current.logicalRange;
        const rangeLen = logicalRange.to - logicalRange.from;
        const px = ((lastIdx - logicalRange.from) / rangeLen) * (cw - cur_pAxisW);
        const cwScaleX = (cw - cur_pAxisW) / rangeLen;
      ...
      const gridUniforms = new Float32Array([
        ...
        offsetX, offsetY,
        cur_pAxisW, cur_timeAxisH, livePixelY, lastPixelX,
        lcR, lcG, lcB, 1.0
      ]);
    ...
    if (gridSegmentsCount > 0) {
      ...
      timeAxisWinners.forEach(({ x }) => {
        pushThickLine({ x, y: 0 }, { x, y: timeAxisY }, 1, gridLineColor);
        pushThickLine({ x, y: timeAxisY }, { x, y: timeAxisY + 4 * dpr }, 1, gridLineColor); // Tick mark
      });
      priceAxisWinners.forEach(({ y }) => {
        pushThickLine({ x: 0, y }, { x: cw - pAxisW, y }, 1, gridLineColor);
        pushThickLine({ x: cw - pAxisW, y }, { x: cw - pAxisW + 4 * dpr, y }, 1, gridLineColor); // Tick mark
      });

      // Axis Borders (drawn slightly darker/thicker depending on theme)
      pushThickLine({ x: cw - pAxisW, y: 0 }, { x: cw - pAxisW, y: timeAxisY }, 1, axisBorderColor);
      pushThickLine({ x: 0, y: timeAxisY }, { x: cw - pAxisW, y: timeAxisY }, 1, axisBorderColor);
    ...
    // ── 3. DRAW HOVER CROSSHAIR LINES & TAG BACKGROUNDS NATIVELY (BEFORE TEXT) ──
    if (gpu.current.drawingPipeline && candles && candles.length > 0) {
      // Allocate generous headroom (64 primitives * 36 floats = 2304 floats) to prevent out-of-bounds RangeError
      const overlayData = new Float32Array(64 * 36);
      let ovPtr = 0;

      const pushRect = (x1, y1, x2, y2, color) => {
        if (ovPtr + 36 > overlayData.length) return;
        overlayData[ovPtr++] = x1; overlayData[ovPtr++] = y1;
        overlayData[ovPtr++] = color[0]; overlayData[ovPtr++] = color[1]; overlayData[ovPtr++] = color[2]; overlayData[ovPtr++] = color[3];

        overlayData[ovPtr++] = x2; overlayData[ovPtr++] = y1;
        overlayData[ovPtr++] = color[0]; overlayData[ovPtr++] = color[1]; overlayData[ovPtr++] = color[2]; overlayData[ovPtr++] = color[3];

        overlayData[ovPtr++] = x1; overlayData[ovPtr++] = y2;
        overlayData[ovPtr++] = color[0]; overlayData[ovPtr++] = color[1]; overlayData[ovPtr++] = color[2]; overlayData[ovPtr++] = color[3];
        ...
        overlayData[ovPtr++] = x2; overlayData[ovPtr++] = y2;
        overlayData[ovPtr++] = color[0]; overlayData[ovPtr++] = color[1]; overlayData[ovPtr++] = color[2]; overlayData[ovPtr++] = color[3];
      };
      ...
      const pushLine = (x1, y1, x2, y2, color) => {
        ...
        overlayData[ovPtr++] = x1 - nx; overlayData[ovPtr++] = y1 - ny;
        overlayData[ovPtr++] = color[0]; overlayData[ovPtr++] = color[1]; overlayData[ovPtr++] = color[2]; overlayData[ovPtr++] = color[3];

        overlayData[ovPtr++] = x1 + nx; overlayData[ovPtr++] = y1 + ny;
        overlayData[ovPtr++] = color[0]; overlayData[ovPtr++] = color[1]; overlayData[ovPtr++] = color[2]; overlayData[ovPtr++] = color[3];

        overlayData[ovPtr++] = x2 - nx; overlayData[ovPtr++] = y2 - ny;
        overlayData[ovPtr++] = color[0]; overlayData[ovPtr++] = color[1]; overlayData[ovPtr++] = color[2]; overlayData[ovPtr++] = color[3];
      ...
      const hoverBgColor = darkMode ? [42/255, 46/255, 57/255, 1.0] : [224/255, 227/255, 235/255, 1.0];

      // Live Price Tag Background
      let livePixelY = -1.0;
      let isUp = true;
      if (candles.length > 0) {
        const lastC = candles[candles.length - 1];
        isUp = lastC.close >= lastC.open;
        const minP = vState.current.priceRange.min;
        const maxP = vState.current.priceRange.max;
        const priceScale = (maxP - minP) > 0 ? timeAxisY / (maxP - minP) : 1;
        livePixelY = timeAxisY - ((lastC.close - minP) * priceScale);
      ...
      if (livePixelY >= 0 && livePixelY <= timeAxisY) {
        ...
        pushRect(cw - pAxisW, livePixelY - 10 * dpr, cw, livePixelY + 10 * dpr, col);
      }

      // Hover Crosshairs & Tooltip Backgrounds
      const hoverPixel = vState.current.hoverPixel;
      if (hoverPixel) {
        const hx = hoverPixel.x * dpr;
        const hy = hoverPixel.y * dpr;
        ...
        if (hx >= 0 && hx <= cw - pAxisW && hy >= 0 && hy <= timeAxisY) {
          ...
          // Hover price label background on the right price axis
          pushRect(cw - pAxisW, hy - 11 * dpr, cw, hy + 11 * dpr, hoverBgColor);

          // Hover time label background on the bottom time axis
          const currentH = (vState.current.timeAxisH || 28) * dpr;
          pushRect(hx - 60 * dpr, timeAxisY, hx + 60 * dpr, timeAxisY + currentH, hoverBgColor);
        }
      }

      if (ovPtr > 0) {
        const finalData = new Float32Array(overlayData.buffer, 0, ovPtr);
        const byteLen = finalData.byteLength;
        gpu.current.device.queue.writeBuffer(gpu.current.drawingBuffer, drawingBufferOffset, finalData.buffer, finalData.byteOffset, byteLen);

        renderPass.setPipeline(gpu.current.drawingPipeline);
        renderPass.setBindGroup(0, gpu.current.drawingBindGroup);
        renderPass.setVertexBuffer(0, gpu.current.drawingBuffer, drawingBufferOffset);
        renderPass.draw(ovPtr / 6);
    ...
    if (gpu.current.drawingPipeline && candles && candles.length > 1 && visualIndicators && visualIndicators.length > 0) {
      const pAxisW_ind = (vState.current.pAxisW || 65) * dpr;
      const timeAxisY_ind = ch - ((vState.current.timeAxisH || 28) * dpr);
  ...
  useEffect(() => {
    ...
    const onPointerDown = (e) => {
      ...
      if (!isAxisClick && activeTool === 'eraser') {
        ...
        const px = (time) => {
          const idx = timeToIndex(time, candles);
          const pAxisW_ind = vState.current.pAxisW || 65;
          return ((idx - logicalRange.from) / rangeLen) * (cw - pAxisW_ind);
        };
      ...
      if (!isAxisClick && activeTool && activeTool !== 'cursor') {
        ...
        const logicalRange = vState.current.logicalRange;
        const rangeLen = logicalRange.to - logicalRange.from;
        const pAxisW_draw = vState.current.pAxisW || 65;
        const idx = logicalRange.from + ((px / (cw - pAxisW_draw)) * rangeLen);
    ...
    const onPointerMove = (e) => {
      if (activeTool && activeTool !== 'cursor' && gpu.current.activeDrawStart) {
        ...
        const logicalRange = vState.current.logicalRange;
        const rangeLen = logicalRange.to - logicalRange.from;
        const idx = logicalRange.from + ((px / (cw - (vState.current.pAxisW || 65))) * rangeLen);
      ...
      if (!isDragging && (!activeTool || activeTool === 'cursor')) {
        ...
        const centerX = cw - (vState.current.pAxisW || 65) + ((vState.current.pAxisW || 65) - textWidthApprox) / 2.0;
      ...
      if (vState.current.manualPriceScale) {
        ...
        const priceScale = (ch - (vState.current.timeAxisH || 28)) / pRange;
  ...
  if (gpuError) {
    return (
      <div className="w-full h-full flex items-center justify-center bg-[#131722] text-sm font-medium p-4 text-center">
        <span className="w-4 h-4 mr-2 border-2 border-amber-400 border-t-transparent rounded-full animate-spin"></span>
        <span className="text-amber-300">Switching to WebGL Engine...</span>
      </div>
    );
  }
  ...
  return (
    <div ref={containerRef} className="w-full h-full relative bg-[#131722] overflow-hidden cursor-crosshair">
      ...
      <div className="absolute top-2 left-2 z-10 pointer-events-none flex items-center space-x-2 bg-slate-950/80 backdrop-blur border border-cyan-500/30 px-2.5 py-1 rounded text-[11px] font-mono text-cyan-300 shadow-lg">
        <span className="w-2 h-2 rounded-full bg-emerald-400 animate-pulse"></span>
        <span>WEBGPU HARDWARE: {gpuDeviceName}</span>
      </div>
    </div>
  );
});
