...
const WebGLChartEngine = forwardRef(({
  ...
  const drawAll = (isPanOnly = false) => {
    ...
    // ══════════════════════════════════════════════════════════════════════
    // ADVANCED 3-PHASE TIME-AXIS COLLISION ENGINE (Extracted to utility)
    ...
    // ── Phase 3: Draw in x-order // ── Phase 3: Draw in x-order ──────────────────────────────────────
    timeAxisWinners.forEach(({ x, label, priority, isMajor }) => {
        ...
        // Grid line
        if (isMajor) {
          gridLayerRef.current.moveTo(x, 0).lineTo(x, timeAxisY);
          gridLayerRef.current.stroke({ width: 1, color: gridColor, alpha: gridAlpha });
        } else {
          drawDashedLine(gridLayerRef.current, x, 0, x, timeAxisY, 1, 4);
