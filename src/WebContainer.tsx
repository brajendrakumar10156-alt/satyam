...
export default function WebContainer({ onLogout, onBackToCoins }: { onLogout?: () => void, onBackToCoins?: () => void }) {
  ...
  const LeftToolbar = ({ horizontal = false }) => {
    ...
    return (
      <div className={`hidden md:flex w-12 shrink-0 border-r ${t.border} ${t.bg} flex-col items-center py-2.5 gap-1.5 z-40 relative select-none max-h-[calc(100vh-60px)] overflow-y-auto scrollbar-none`}>
        {categories.map((cat) => {
