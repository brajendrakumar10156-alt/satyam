...
function coinIconUrl(symbol, tier = 0) {
...
function handleCoinIconError(e, coin) {
  const currentTier = parseInt(e.target.getAttribute('data-tier') || '0');
  if (currentTier < 3) {
    const nextTier = currentTier + 1;
    e.target.setAttribute('data-tier', nextTier);
...
export default function App({ onLogout, onBackToCoins }) {
  ...
  useEffect(() => {
    const worker = new Worker(new URL('./indicatorWorker.js', import.meta.url), { type: 'module' });
  ...
  useEffect(() => {
    if (!chartRef.current || useWebGL) return;
  ...
  const handlePointerDown = (e) => {
    ...
    if (!useWebGL && chartInstance.current) { chartInstance.current.applyOptions({ handleScroll: false, handleScale: false }); }
  ...
  const handlePointerUp = () => {
    setHoverCoords(null);
    if (!useWebGL && chartInstance.current) {
                            ...
                            onVisibleRangeChange={(range) => {
                              if (range?.from && range?.to) {
                                if (allCandles.length > 0) {
                                  ...
                                  // If we pan to the left, within 100 candles of the oldest data
