...
if (drawOnCanvasStart !== -1 && requestDrawEnd !== -1) {
  // Remove everything from drawOnCanvas to the end of requestDraw
  lines.splice(drawOnCanvasStart, requestDrawEnd - drawOnCanvasStart + 1, 
...
for (let i = 0; i < lines.length; i++) {
  if (lines[i].includes("<canvas ref={canvasRef}")) {
    lines.splice(i, 1, ...pixiLayerCode);
  }
}
