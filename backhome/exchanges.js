...
function toOkxInstId(unified) {
  const { base, quote } = parseUnifiedSymbol(unified);
  return `${base}-${quote}`;
}
...
export async function fetchExchangeSymbols(exchangeId) {
  const cacheKey = `cadpro_symbols_cache_${exchangeId}`;
  try {
...
async function fetchBybitCandles(symbol, interval, limit, before, signal) {
  ...
  let url = `https://api.bybit.com/v5/market/kline?category=spot&symbol=${symbol}&interval=${intv}&limit=${Math.min(limit, 1000)}`;
...
export async function fetchExchangeCandles(exchangeId, symbol, interval, limit = 1000, before = null) {
  ...
  try {
    // Race backend and public CORS proxy in parallel: fastest wins!
    return await Promise.any([fetchBackend(), fetchProxy()]);
  } catch (err) {
    throw new Error(`Failed to fetch candles for ${exchangeId} ${symbol} via backend and proxy: ${err.message}`);
  }
}
